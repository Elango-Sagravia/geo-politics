...
export async function GET(request) {
  ...
  try {
    ...
    const checkSubscriberQuery = `
      SELECT * FROM subscribers WHERE user_id = $1 AND website_id = 3;
    `;
    const subscriberResult = await query(checkSubscriberQuery, [userId]);

    if (subscriberResult.rows.length > 0) {
      // The user is already in the subscribers table for website_id = 3, update the status to 'subscribed'
      const updateSubscriberQuery = `
        UPDATE subscribers
        SET status = 'subscribed'
        WHERE user_id = $1 AND website_id = 3;
      `;
      await query(updateSubscriberQuery, [userId]);
    } else {
      // The user is not subscribed to website_id 3, insert a new row with created_at
      const insertSubscriberQuery = `
        INSERT INTO subscribers (user_id, website_id, status, created_at)
        VALUES ($1, 3, 'subscribed', NOW());
      `;
